...
export function PluginRegistry() {
  ...
  if (!plugins || plugins.length === 0) {
    return (
      <div className="py-16 text-center text-slate-400">
        <Puzzle size={36} className="mx-auto mb-3 text-slate-300 animate-bounce" />
        <p className="text-base font-semibold text-slate-800">No plugins registered in this environment</p>
        <p className="mt-1 text-sm text-slate-400">Deploy first-party packages to seed the extension registry.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 p-6 animate-in fade-in duration-200 bg-white">
      ...
      {filteredPlugins.length === 0 ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredPlugins.map((plugin) => {
            ...
            const badgeColor = CATEGORY_COLORS[cat] ?? 'bg-slate-50 text-slate-700 border-slate-200';
            ...
            return (
              <Card
                ...
                className="group cursor-pointer bg-white border-slate-200 rounded-xl overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
              >
                <CardContent className="p-5 flex flex-col h-full justify-between gap-4">
                  <div className="space-y-3">
                    {/* Header: custom icon and status badge */}
                    <div className="flex items-start justify-between">
                      <div className="w-10 h-10 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-xl flex-shrink-0 group-hover:bg-indigo-50 group-hover:border-indigo-100 transition-colors">
                        {catIcon}
                      </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-400 font-mono line-clamp-1">{plugin.package_name}</p>
                    </div>
                  ...
                  <div className="border-t border-slate-100 pt-3.5 mt-auto flex items-center justify-between text-[10px] font-semibold font-mono text-slate-400">
                    <span className="flex items-center gap-1">
                      <Database size={11} className="text-slate-300" />
                      v{plugin.version}
                    </span>
                    
                    <span className="flex items-center gap-1 bg-slate-50 border border-slate-200 text-slate-500 px-2 py-0.5 rounded-md font-bold font-sans">
