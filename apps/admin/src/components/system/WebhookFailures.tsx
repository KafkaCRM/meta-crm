  ...
  if (isLoading) {
    return (
      <div className="py-12 flex flex-col items-center justify-center gap-2">
        ...
        <span className="text-xs text-slate-400">Loading failure registry…</span>
      </div>
    );
  }
        ...
        <div className="flex flex-wrap items-center gap-2">
          {selectedIds.length > 0 && (
            <Button
              variant="outline"
              size="sm"
              className="h-8 text-xs gap-1 border-indigo-200 text-indigo-700 bg-indigo-50/50 hover:bg-indigo-50 rounded-lg transition-all"
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="bg-white border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <CardHeader className="pb-3 border-b border-slate-100">
              <div>
                <CardTitle className="text-base font-semibold text-slate-900">Webhook Dispatcher Failures</CardTitle>
                      ...
                      {filteredFailures.map((failure) => (
                        <tr
                          ...
                          className={`hover:bg-slate-50/50 cursor-pointer transition-colors ${
                            selectedFailure?.id === failure.id ? 'bg-indigo-50/20' : ''
                          }`}
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className="inline-flex items-center text-xs font-semibold px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 font-mono">
                              {failure.event_type}
                            </span>
                          </td>
            ...
            <Card className="bg-[#0b0f19] border-slate-800 text-slate-100 rounded-xl shadow-lg relative overflow-hidden animate-in slide-in-from-right duration-200">
              <CardHeader className="border-b border-slate-800 pb-3 bg-slate-900/40">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-mono tracking-widest text-indigo-400">Webhook Diagnostics Logs</span>
                  </div>
                  <button
              ...
              <CardContent className="pt-4 space-y-4 text-sm">
                ...
                <div className="bg-slate-950 p-3 rounded-lg border border-slate-900 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono text-slate-500 uppercase">Destination URL</span>
                ...
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Request Headers</p>
                  <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-900 font-mono text-[9px] text-slate-300 space-y-1">
                    <div><span className="text-slate-500">Content-Type:</span> application/json</div>
                    <div><span className="text-slate-500">X-Meta-CRM-Signature:</span> sha256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855</div>
                    <div><span className="text-slate-500">X-Meta-Tenant-ID:</span> {selectedFailure.tenant_id}</div>
                    <div><span className="text-slate-500">User-Agent:</span> MetaCRM-WebhookDispatcher/2.0</div>
                  </div>
                </div>

                {/* Collapsible JSON Event Payload */}
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Event JSON Payload</p>
                ...
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Server Exception logs</p>
            ...
            <Card className="bg-white border-slate-200 rounded-xl shadow-sm">
              <CardHeader className="pb-3 border-b border-slate-100">
                <div className="flex items-center gap-1.5">
                  ...
                  <CardTitle className="text-sm font-bold text-slate-900 uppercase tracking-wide">Webhook Logs Info</CardTitle>
                </div>
                <CardDescription className="text-xs text-slate-400">Real-time status overview</CardDescription>
              </CardHeader>
              <CardContent className="pt-4 space-y-4 text-xs">
                <div className="flex items-center justify-between py-1 border-b border-slate-50">
                  <span className="font-semibold text-slate-600">Active Handlers</span>
                ...
                <div className="flex items-center justify-between py-1 border-b border-slate-50">
                  <span className="font-semibold text-slate-600">Retries Schedule</span>
                  <span className="text-slate-900 font-semibold">Exponential Backoff (5 attempts)</span>
                </div>
                <div className="flex items-center justify-between py-1 border-b border-slate-50">
                  <span className="font-semibold text-slate-600">Connection Timeout</span>
                  <span className="text-slate-900 font-semibold">10,000ms</span>
                </div>
                <div className="flex items-center justify-between py-1">
                  <span className="font-semibold text-slate-600">Avg Success Rate</span>
                  <span className="text-slate-500 font-mono">99.85%</span>
                </div>
