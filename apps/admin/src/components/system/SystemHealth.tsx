    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <Card className="bg-[#0b0f19] border-slate-800 text-slate-100 rounded-xl overflow-hidden shadow-lg relative">
        <div className="absolute top-0 right-0 p-6 opacity-10">
          <Cpu size={120} className="text-slate-400" />
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase font-mono tracking-widest text-indigo-400 font-bold">Diagnostics Console</span>
              </div>
              <h2 className="text-xl font-bold font-mono text-white">Platform System Health</h2>
              <p className="text-xs text-slate-400">Real-time developer diagnostics, job queue pipelines, and webhook delivery records.</p>
            </div>
            
            <div className="flex flex-wrap gap-4 font-mono text-xs">
              <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800/80 flex flex-col min-w-[120px]">
                <span className="text-slate-500 text-[10px] uppercase font-semibold">Active Workers</span>
                <span className="text-emerald-400 font-bold mt-1 text-sm">4 Online</span>
              </div>
              <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800/80 flex flex-col min-w-[120px]">
                <span className="text-slate-500 text-[10px] uppercase font-semibold">Queue Ingestion</span>
                <span className="text-white font-bold mt-1 text-sm">99.98% OK</span>
              </div>
              <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800/80 flex flex-col min-w-[120px]">
                <span className="text-slate-500 text-[10px] uppercase font-semibold">Webhook Latency</span>
                <span className="text-indigo-400 font-bold mt-1 text-sm">242ms</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex border-b border-slate-200">
        ...
        >
          ...
          <Badge className="ml-1 bg-indigo-50 text-indigo-700 text-[10px] hover:bg-indigo-50 border-transparent shadow-none px-1.5 py-0">
            Active
          </Badge>
        </button>
        <button
