    ...
    return (
      <Card className={`bg-white border-[#d3cec6] rounded-xl shadow-none ${className ?? ''}`}>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-[#111111]">Follow-ups Today</CardTitle>
        </CardHeader>
      ...
      <CardContent className="pt-4">
        {followUps.length === 0 ? (
          <div className="flex flex-col items-center py-8 text-center">
            ...
            <p className="text-sm font-medium text-[#111111]">No follow-ups scheduled for today</p>
            <p className="text-xs text-[#9c9fa5] mt-1">Enjoy the quiet!</p>
          </div>
        ) : (
          <div className="space-y-2">
            {followUps.map((fu) => (
              <div
                key={fu.id}
                className="flex items-center justify-between rounded-lg border border-[#ebe7e1] p-3"
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <span className={CHANNEL_COLORS[fu.channel] ?? 'text-[#9c9fa5]'}>
                    {CHANNEL_ICONS[fu.channel] ?? <Clock size={12} />}
                  </span>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#111111] truncate">{fu.party_name}</p>
                    <p className="text-xs text-[#9c9fa5] capitalize">{fu.type}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 ml-3 shrink-0">
                  <span className="text-xs text-[#9c9fa5]">{fu.time}</span>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-6 w-6 p-0 text-[#626260] hover:text-[#111111]"
                    title="Call"
                  >
                    <Phone size={12} />
                  </Button>
